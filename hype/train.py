...
def train(
    ...
    for epoch in range(opt.epoch_start, opt.epochs):
        # print('The model before the epoch is: {}'.format(model))
        # print('The model before the epoch is: {}'.format(model.lt[0].weight))
        epoch_loss.fill_(0)
        data.burnin = False
        lr = opt.lr
        ...
        for i_batch, (inputs, targets) in enumerate(loader_iter):
            ...
            loss.backward()
            optimizer.step(lr=lr, counts=counts, complex_tensor=complex_tensor)
