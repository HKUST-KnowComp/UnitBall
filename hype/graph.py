...
def reconstruction_worker(adj, model, objects, progress=False, complex=False, embeddings=None, hits_conf=[1, 3, 10]):
    ranksum = nranks = ap_scores = iters = mrr = roc = 0
    labels = np.empty(embeddings[0].size(0)) if complex else np.empty(model.lt.weight.size(0))
    ...
    for object in tqdm(objects) if progress else objects:
        ...
        if complex:
            dists = model.complex_energy(embeddings[0][None, object], embeddings[1][None, object], embeddings[0], embeddings[1])
        else:
        ...
        labels[neighbors] = 1
        roc += roc_auc_score(labels, -dists.detach().cpu().numpy())
        ap_scores += average_precision_score(labels, -dists.detach().cpu().numpy())

        iters += 1
    return np.array((float(ranksum), nranks, ap_scores, iters, mrr, roc) + tuple(hits_ary))


def eval_reconstruction(adj, model, workers=1, progress=False, complex=False, embeddings=None):
    '''
    ...
    if workers > 1:
        with ThreadPool(workers) as pool:
            f = partial(reconstruction_worker, adj, model, complex=complex, embeddings=embeddings)
            results = pool.map(f, np.array_split(objects, workers))
            results = np.array(results).sum(axis=0).astype(float)
    else:
        results = reconstruction_worker(adj, model, objects, progress=True, complex=complex, embeddings=embeddings)
    return float(results[0] / results[1]), float(results[2] / results[3]), float(results[4] / results[3]), float(results[5] / results[3]), results[6:] / results[3]
